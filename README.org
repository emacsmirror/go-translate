...
#+begin_src emacs-lisp
  (defvar lnr-ai-oneshot-prompts
    (list "Polish the text" "Fix the errors in code"))

  (defvar lnr-ai-oneshot-models
    (list "deepseek-chat" "gpt-4o-mini" "gemini-2.5-pro"))

  (defvar lnr-ai-oneshot-last-model nil)

  (defvar lnr-ai-oneshot-history nil)

  (defun lnr-ai-oneshot ()
    "Use C-. C-, to switch model."
    (interactive)
    (require 'go-translate)
    (let ((prompt nil)
          (model (or lnr-ai-oneshot-last-model
                     (setq lnr-ai-oneshot-last-model (or (car lnr-ai-oneshot-models) gt-chatgpt-model)))))
      (cl-flet ((get-prompts ()
                  (cl-delete-duplicates
                   (append lnr-ai-oneshot-history lnr-ai-oneshot-prompts) :from-end t :test #'equal))
                (change-model (&optional prev)
                  (let* ((pos (or (cl-position lnr-ai-oneshot-last-model lnr-ai-oneshot-models :test #'equal) -1))
                         (next (if prev (max 0 (1- pos)) (min (1- (length lnr-ai-oneshot-models)) (1+ pos)))))
                    (setq lnr-ai-oneshot-last-model (nth next lnr-ai-oneshot-models))
                    (setq model lnr-ai-oneshot-last-model)
                    (overlay-put (car (overlays-at 1)) 'after-string lnr-ai-oneshot-last-model))))
        (setq prompt
              (minibuffer-with-setup-hook
                ...
                (completing-read "Prompt (): " (get-prompts) nil nil nil 'lnr-ai-oneshot-history)))
        (gt-start (gt-translator
                   :taker (gt-taker
                           :text 'point :pick nil
...
This is used to read current buffer sentence by sentence within Emacs:
- The core command is =lnr-speak-buffer=. Start or restart reading from any position of buffer with this command
- Stop reading with command =lnr-speak-buffer-interrupt= or just pressing =C-g=
- Customize your TTS with =lnr-speak-buffer-engine/lnr-speak-buffer-step-action= and others
- This is just an example; for me, it's enough; feel free to modify and extend it to suit your needs

Just have a try. It is asynchronously with caching, and quite practical to reading articles and novels in Emacs:
#+begin_src emacs-lisp
  (defvar lnr-speak-buffer-task nil)
  (defvar lnr-speak-buffer-engine 'native)
  (defvar lnr-speak-buffer-interval 0.1)
  (defvar lnr-speak-buffer-face 'font-lock-warning-face)
  (defvar lnr-speak-buffer-step-pred #'<)
  (defvar lnr-speak-buffer-step-final nil)

  (defvar lnr-speak-buffer-step-action
    (lambda ()
      (let ((beg (point)))
        (forward-sentence)
        (skip-syntax-forward ".")
  ...
  (defvar lnr-speak-buffer-text-filter
    (lambda (text) (replace-regexp-in-string "[ \t\n\r]" "" text)))

  (defvar-keymap lnr-speak-buffer-map
    "<mouse-3>" 'lnr-speak-buffer-interrupt "C-g" 'lnr-speak-buffer-interrupt)

  (defun lnr-speak-buffer-interrupt ()
    (interactive)
    (when lnr-speak-buffer-task
      (pdd-signal lnr-speak-buffer-task 'cancel)
      (setq lnr-speak-buffer-task nil)))

  (defun lnr-speak-buffer ()
    (interactive)
    (require 'go-translate)
    (lnr-speak-buffer-interrupt)

    (let ((buf (current-buffer))
          (ov (make-overlay 1 1 nil nil t)))
      (overlay-put ov 'face lnr-speak-buffer-face)
      (overlay-put ov 'keymap lnr-speak-buffer-map)
             ...
             (if-let* ((bounds-list
                        (save-excursion
                          ...
                          (cl-loop repeat 3 ; 3 = 1 + 2: read 1, cache 2
                                   ...
                                   for end = (save-excursion (funcall lnr-speak-buffer-step-action) (point))
                                   while (funcall lnr-speak-buffer-step-pred beg end)
                                   collect (cons beg end)
                                   do (funcall lnr-speak-buffer-step-action))))
                       (text-list (mapcar
                                   (lambda (bds)
                                     (funcall (or lnr-speak-buffer-text-filter #'identity)
                 ...
                 (pdd-chain (car text-list)
                   (lambda (text)
                     (with-current-buffer buf
                       ...
                       (mapc (lambda (c)
                               (let ((pdd-fail #'ignore))
                                 (gt-speech lnr-speak-buffer-engine c 'zh #'ignore)))
                             (cdr text-list))
                       ;; 2. play the current
                       (setq lnr-speak-buffer-task (gt-speech lnr-speak-buffer-engine text 'zh))))
                   (lambda (_)
                     (with-current-buffer buf
                       ;; 3. next loop
                       (pdd-cacher-clear gt-tts-cache-store)
                       (move-overlay ov (cdr current) (cdr current))
                       (setq lnr-speak-buffer-task
                             (pdd-delay (if (eq (char-after (cdr current)) ?\n)
                                            (* 3 lnr-speak-buffer-interval) ; more delay time for paragraph end
                                          lnr-speak-buffer-interval)
                               (lambda ()
               ...
               (delete-overlay ov)
               (setq lnr-speak-buffer-task nil)
               (if (functionp lnr-speak-buffer-step-final)
                   (funcall lnr-speak-buffer-step-final)
