...
在 Emacs 内逐段阅读当前 buffer 内容 (就像某些阅读 App 中的那样):
- 核心命令是 =lnr-speak-buffer=, 在 buffer 内任意位置，调用它开始或重新开始阅读
- 如果要停止播报，执行 =lnr-speak-buffer-interrupt= 命令或按下 =C-g=
- 可以通过 =lnr-speak-buffer-engine/lnr-speak-buffer-step-action= 等变量对阅读进行设置
- 这只是个示例; 对于我来说足够好用了; 任何人都可以依照自己需求对它进行调整和扩展
